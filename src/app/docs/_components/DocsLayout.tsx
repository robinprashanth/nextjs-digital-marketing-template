...
const sidebarItems = [
  ...
  {
    title: "Guides",
    items: [{ href: "/docs/deployment", title: "Deployment" }],
  },
];
      ...
      <div className="space-y-6">
        {sidebarItems.map((section, i) => (
          <div key={i} className="space-y-2">
            <h4 className="text-sm font-medium text-theme-neutral-500">
              {section.title}
            </h4>
...
export function DocsLayout({ children }: DocsLayoutProps) {
  return (
    <div className="flex min-h-screen">
      ...
      {/* Desktop Sidebar */}
      <aside className="fixed left-0 top-0 hidden h-screen w-[240px] border-r border-border bg-background md:block">
        <DocsSidebar />
      </aside>

      {/* Main Content */}
      <main className="ml-0 flex-1 md:ml-[240px]">
